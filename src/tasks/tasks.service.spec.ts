...
describe('TasksService', () => {
 ...
 const fakeExercise = { exercise_id: 'e-1', language_id: 1 };
 ...
 const fakeTask = {
   ...
   position: 0,
   created_at: new Date(),
   question_pool: { question: fakeQuestion.question, language_id: fakeQuestion.language_id },
   task_words: [],
 };
 ...
 it('create() prüft, dass die Frage zur Sprache der Exercise passt, und setzt position standardmäßig auf 0', async () => {
   prisma.question_pool.findUnique.mockResolvedValue(fakeQuestion);
   ...
   await service.create('e-1', { question_id: 'q-1' });

   expect(prisma.tasks.create.mock.calls[0][0].data.position).toBe(0);
 });

 it('create() wirft BadRequestException, wenn Frage- und Exercise-Sprache nicht übereinstimmen', async () => {
   prisma.question_pool.findUnique.mockResolvedValue({ ...fakeQuestion, language_id: 2 });
   prisma.exercises.findUnique.mockResolvedValue(fakeExercise);

   await expect(service.create('e-1', { question_id: 'q-1' })).rejects.toBeInstanceOf(
     BadRequestException,
   );
   expect(prisma.tasks.create).not.toHaveBeenCalled();
 });

 it('findAllForExercise() sortiert nach position aufsteigend und gibt kein correct_answer zurück', async () => {
   prisma.tasks.findMany.mockResolvedValue([fakeTask]);

   const result = await service.findAllForExercise('e-1');
   ...
   expect(result[0].question).toBe(fakeQuestion.question);
 });
